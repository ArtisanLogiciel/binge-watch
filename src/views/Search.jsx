    ...
    return (
        <Layout onClick={()=>setFocus(false)}>
            <div className={`fixed top-18 h-25 w-screen bg-grey-dark z-20 overflow-hidden`}>
                <div className={`absolute h-56 w-screen bg-gradient-to-t from-grey-light/25 via-grey-light/15 transition-all duration-500 ${focus ? '-top-25' : 'top-0'}`} />
                    <input type="text"
                           ...
                           value={valToSearch}
                    />
                <div className={`absolute right-[3.5vw] top-6 animate-appear z-30 ${valToSearch || 'animate-disappear'}`}>
                    <button className={`h-12 w-12 text-white ${focus ? 'opacity-80' : 'opacity-50' } transition-all duration-500 `}
                    onClick={()=>{setValToSearch("")}}
                            role='clear' aria-hidden={valToSearch===""} aria-label="Effacer le champ de recherche"
                    >
                        <svg stroke="none" fill="currentcolor" version="1.1" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className={`py-43 px-[3.5vw] flex flex-wrap h-full mx-auto`}
                ...
                {valToSearch.length>1 ? <Results valToSearch={valToSearch} /> : <>liste des catégories</>}
...
function Results({valToSearch}) {
    ...
    return (
        data ? <>
            {data.results?.map((res, index)=>{ return (
                <div key={index}>
                    <Card movie={res}/>
                </div>
                            )
